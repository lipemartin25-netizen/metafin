...
const PATTERNS = {
    ...
    'Missing rel=noopener': /target=['"]_blank['"](?!\s+rel=['"]noopener['"])/i,
};
        ...
        if (stat.isDirectory()) {
            ...
            lines.forEach((line, index) => {
                for (const [name, regex] of Object.entries(PATTERNS)) {
                    if (regex.test(line)) {
                        // Ignore some known safe patterns or comments
...
if (issues.length > 0) {
    ...
    console.log("\n❌ Falha na auditoria estática.");
    process.exit(1);
} else {
    console.log("\n✅ Auditoria concluída. Nenhum padrão inseguro detectado.");
}
