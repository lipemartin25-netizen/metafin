...
export default function TaxPlanner({ financialData }) {
 ...
 <div className="relative overflow-hidden flex flex-col items-center justify-center rounded-[2.5rem] bg-surface-primary from-blue-600 via-indigo-600 to-brand-dark p-10 md:p-14 text-content-primary shadow-card border border-[var(--border)] text-center group perspective-1000">
 {/* Efeitos 3D Internos */}
 ...
 <Shield className="w-16 h-16 text-blue-200 drop-shadow-card animate-bounce-slow" />
 </div>
 ...
 Pare de deixar dinheiro na mesa. Nossa <strong className="text-content-primary drop-shadow-lg shadow-black/10">Inteligência Estratégica</strong> cruza suas deduções
 (PGBL, Saúde, Escola) e escaneia a rota matemática exata para você pagar o mínimo legal de imposto de renda.
 </p>

 {/* Decorative border highlight */}
 <div className="absolute inset-x-0 h-1 bg-surface-primary from-transparent via-blue-300 to-transparent bottom-0 opacity-0 group-hover:opacity-100 animate-[scan_2s_ease-in-out_infinite_reverse] shadow-card" />
 </div>
 ...
 <input
 type="number"
 className="w-full bg-gray-800/30 dark:bg-black/20 border border-gray-700/40 dark:border-[var(--border)] rounded-xl px-3 py-2 text-sm text-content-primary dark:text-content-primary outline-none focus:border-blue-500"
 ...
 className="w-full bg-gray-800/30 dark:bg-black/20 border border-gray-700/40 dark:border-[var(--border)] rounded-xl px-3 py-2 text-sm text-content-primary dark:text-content-primary outline-none focus:border-blue-500 mt-1"
 ...
 <h3 className="font-bold text-content-primary dark:text-content-primary flex items-center gap-2 mb-6">
 Comparativo Anual <ArrowRight className="w-4 h-4 text-gray-500" />
 </h3>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className={`p-5 rounded-2xl border transition-all ${result.bestModel === 'simplificado' ? 'bg-blue-50 dark:bg-blue-500/10 border-blue-500 shadow-lg shadow-black/10 ring-2 ring-blue-500/50' : 'bg-gray-800/30 dark:bg-gray-800/40/[0.02] border-gray-700/40 dark:border-[var(--border)] opacity-70'}`}>
 {result.bestModel === 'simplificado' && <CheckCircle2 className="w-6 h-6 text-blue-500 mb-2" />}
 <h4 className="font-bold text-content-primary dark:text-content-primary text-lg">Modelo Simplificado</h4>
 <p className="text-xs text-gray-500 mt-1 mb-4 h-8">Usa um desconto padrão de 20% limitado ao teto, ignorando recibos.</p>
 <div className="pt-4 border-t border-gray-700/40 dark:border-[var(--border)]">
 <p className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Imposto Total Devido</p>
 <p className={`text-2xl font-black ${result.bestModel === 'simplificado' ? 'text-blue-600 dark:text-blue-400' : 'text-content-primary dark:text-content-primary'}`}>{formatCurrency(result.irSimplificada)}</p>
 </div>
 </div>

 <div className={`p-5 rounded-2xl border transition-all ${result.bestModel === 'completo' ? 'bg-purple-50 dark:bg-brand-primary/10 border-brand-primary shadow-lg shadow-black/10 ring-2 ring-emerald-500/50' : 'bg-gray-800/30 dark:bg-gray-800/40/[0.02] border-gray-700/40 dark:border-[var(--border)] opacity-70'}`}>
 {result.bestModel === 'completo' && <CheckCircle2 className="w-6 h-6 text-brand-primary mb-2" />}
 <h4 className="font-bold text-content-primary dark:text-content-primary text-lg">Modelo Completo</h4>
 ...
 <div className="pt-4 border-t border-gray-700/40 dark:border-[var(--border)]">
 <p className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Imposto Total Devido</p>
 <p className={`text-2xl font-black ${result.bestModel === 'completo' ? 'text-brand-primary dark:text-brand-glow' : 'text-content-primary dark:text-content-primary'}`}>{formatCurrency(result.irCompleta)}</p>
 </div>
 </div>
 </div>
 ...
 <div>
 <h4 className="font-bold text-content-primary dark:text-content-primary text-sm">{sug.action}</h4>
