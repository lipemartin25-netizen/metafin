        ...
        <div className="py-6 space-y-6 animate-fade-in pb-20">
            ...
            <div className="grid md:grid-cols-3 gap-6">
                <div className={`\${tw.card} col-span-2 relative overflow-hidden`}>
                    ...
                    {portfolio.total === 0 && (
                        <p className="text-slate-500 text-sm mt-3">Conecte uma corretora abaixo para ver seu patrimônio.</p>
                    )}
                </div>
                ...
                <div className={`\${tw.card} p-6`}>
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-xs text-slate-400">Comparado ao Ibovespa e CDI</p>
                        </div>
                        <div className="flex gap-4">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] text-slate-400 font-bold uppercase">Ibov</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <div className="w-2.5 h-2.5 rounded-full bg-orange-500" />
                                <span className="text-[10px] text-slate-400 font-bold uppercase">CDI</span>
                            </div>
                        </div>
                    </div>
                ...
                {connectedBrokers.length === 0 ? (
                    <div className={`\${tw.card} text-center py-12 border-dashed border-2 border-white/10`}>
                        ...
                        <p className="text-slate-400 text-sm mb-6 max-w-sm mx-auto">
                            Selecione uma corretora abaixo para conectar via <strong className="text-white">Pluggy Open Finance</strong>.
                        </p>
                    </div>
                ) : (
                                            ...
                                            <div key={idx} className="flex items-center justify-between p-2 hover:bg-gray-800/40/5 rounded-lg transition-colors">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-slate-400">{asset.type}{asset.quantity ? ` · ${asset.quantity} un.` : ''}</p>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <p className="text-sm font-medium text-white">{fmt((asset.price || 0) * (asset.quantity || 1))}</p>
                                                    {asset.price && <p className="text-xs text-slate-400">{fmt(asset.price)}/un</p>}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                    {availableBrokers.filter(b => b.id !== 'openfinance').map(broker => (
                        <button
                            ...
                            className="p-3 rounded-xl bg-gray-800/40/5 hover:bg-gray-800/40/10 border border-white/5 hover:border-brand-primary/30 transition-all flex flex-col items-center gap-2 text-center group"
                        ...
                        >
                            ...
                            <span className="text-[10px] font-medium text-slate-400 group-hover:text-white truncate w-full leading-tight">{broker.name}</span>
                        </button>
                    ))}
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4 mt-6">
                    ...
                    >
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[10px] text-slate-400 mt-0.5">Veja o desempenho histórico do Ibovespa e CDI integrados ao app.</p>
                        </div>
                    </button>

                    <div className={`\${tw.card} p-4 flex items-center gap-4 bg-gradient-to-r from-brand-primary/5 to-transparent border-brand-primary/20`}>
                        <div className="w-12 h-12 rounded-xl bg-brand-primary text-white flex items-center justify-center shadow-lg">
            ...
            {showB3Modal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm animate-fade-in">
                    <div className="bg-[#0d1424] border border-white/15 w-full max-w-2xl rounded-3xl shadow-[0_25px_60px_rgba(0,0,0,0.85)] animate-slide-up overflow-hidden">
                        {/* Header */}
                        ...
                        <div className="grid grid-cols-3 gap-4 p-6 border-b border-white/5">
                            <div className="text-center">
                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">IBOV Atual</p>
                                <p className="text-2xl font-bold text-white">{b3Stats.lastIbov.toLocaleString('pt-BR')}</p>
                                <p className="text-xs text-slate-400">{b3Stats.lastMonth}</p>
                            </div>
                            <div className="text-center">
                            ...
                            <div className="text-center">
                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">CDI 2024</p>
                        ...
                        <div className="p-6">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] text-slate-400 font-bold uppercase">Ibovespa</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <div className="w-2.5 h-2.5 rounded-full bg-orange-400" />
                                    <span className="text-[10px] text-slate-400 font-bold uppercase">CDI Acum. (%)</span>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm animate-fade-in">
                    <div className="bg-[#0d1424] border border-white/15 w-full max-w-sm rounded-3xl overflow-hidden animate-slide-up shadow-[0_25px_60px_rgba(0,0,0,0.85)] relative">
                        ...
                        <div className="p-6 text-center border-b border-white/5 bg-gray-800/40/5">
                            ...
                            <p className="text-xs text-slate-400 mt-1">Escolha como deseja sincronizar</p>
                        </div>
                        <div className="p-6 space-y-3">
                            ...
                            >
                                ...
                                <div>
                                    <p className="text-sm font-bold text-white">Via Open Finance (Pluggy)</p>
                                    <p className="text-[10px] text-slate-400">Sincronização automática · regulado pelo BCB</p>
                                </div>
                            </button>
                            <button
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-400">Inserir ativos manualmente</p>
                                </div>
                            </button>
                        </div>
                    </div>
